...
class AcquisitionEIC(AcquisitionBase):
    """
    ...
    def __init__(self, BOmodel, space, optimizer, cost_withGradients=None, jitter=0.01, threshold=0., classification=False):
        self.optimizer = optimizer
        super(AcquisitionEIC, self).__init__(BOmodel, space, optimizer)
    ...
    def _compute_acq(self, x):
        ...
        for i in range(self.cons_num):
            if not self.classification:
                ...
                cdf = norm.cdf(self.threshold, loc=mean, scale=np.sqrt(var))
                cdf_lower = norm.cdf(self.threshold - self.beta * np.sqrt(var), loc=mean, scale=np.sqrt(var))
                cdf_upper = norm.cdf(self.threshold + self.beta * np.sqrt(var), loc=mean, scale=np.sqrt(var))

                prob = prob * cdf
            else:
                mean, _ = self.model_cons[i].predict(x)
                prob = prob * mean
        # return the product of EI and POF
